...
double nbcm_calculate_separation_cost(
    const int n,
...
void nbcm_first_separation(
    ...
    double min_val = node_weight_second[0] - node_weight_first[0];
    double max_val = min_val;
    for(int i = 1; i < n; i++) {
        const double d = node_weight_second[i] - node_weight_first[i];
        if(min_val > d) {
            min_val = d;
        }
        if(max_val < d) {
    ...
    int *tmp = _2n_buffer;
    for(int i = 0; i < n; i++) {
        const int d = (node_weight_second[i] - node_weight_first[i] - min_val) * MAX_WEIGHT / (max_val - min_val);
        tmp[i * 2] = d;
        tmp[i * 2 + 1] = i;
    }
...
double nbcm_fiduccia_mattheyses_inner(
    const int n,
    const int adjacent_n,
    const int *restrict edge_separator,
    const int *restrict edge,
    ...
    for(int i = 0; i < n; i++) {
        if(belongs[i] == 0) {
            ...
            bucket_gain_first[first_size] = 0.0;
            ...
            bucket_gain_second[second_size] = 0.0;
...
void nbcm_fiduccia_mattheyses(
    ...
    double cost = nbcm_calculate_separation_cost(
        n,
        adjacent_n,
        edge_separator,
        edge,
    ...
    while(1) {
        ...
        double improve = nbcm_fiduccia_mattheyses_inner(
            n,
            adjacent_n,
            edge_separator,
            edge,
...
double nbcm_calculate_single_cost(
    const int cost_type,
    const int distance
) {
    if(distance == 0) {
...
void node_based_cost_minimization_inner(
    const int cost_type,
    ...
    for(int i = 0; i < n + 10; i++) {
        costs[i] = nbcm_calculate_single_cost(cost_type, i);
    }
    ...
    while(meta_size > 0) {
        ...
        for(int i = 0; i < meta_size; i++) {
            ...
            const double node_cost_to_border = border_magnification * nbcm_calculate_cost(&node_wrs, ob, predicted_om, predicted_om, oe);
            ...
            for(int j = 0; j < e - b; j++) {
                ...
                if(adjacent_before_b >= 0) {
                    b += nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, ob, predicted_om);
                    node_weight_first[j] -= nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, ob, predicted_om);
                    node_weight_second[j] -= nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, predicted_om, oe);
                }
                if(adjacent_after_b >= 0) {
                    b += nbcm_calculate_cost(&node_wrs, predicted_om, oe, adjacent_after_b, adjacent_after_e);
                    node_weight_first[j] -= nbcm_calculate_cost(&node_wrs, ob, predicted_om, adjacent_after_b, adjacent_after_e);
                    node_weight_second[j] -= nbcm_calculate_cost(&node_wrs, predicted_om, oe, adjacent_after_b, adjacent_after_e);
                }
            ...
            for(int j = 0; j < adjacent_size; j++) {
                ...
                if(adjacent_before_b >= 0) {
                    b += nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, ob, predicted_om);
                    adjacent_node_weight_first[j] -= nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, ob, predicted_om);
                    adjacent_node_weight_second[j] -= nbcm_calculate_cost(&node_wrs, adjacent_before_b, adjacent_before_e, predicted_om, oe);
                }
                if(adjacent_after_b >= 0) {
                    b += nbcm_calculate_cost(&node_wrs, predicted_om, oe, adjacent_after_b, adjacent_after_e);
                    adjacent_node_weight_first[j] -= nbcm_calculate_cost(&node_wrs, ob, predicted_om, adjacent_after_b, adjacent_after_e);
                    adjacent_node_weight_second[j] -= nbcm_calculate_cost(&node_wrs, predicted_om, oe, adjacent_after_b, adjacent_after_e);
                }
...
void node_based_cost_minimization(
    ...
    node_based_cost_minimization_inner(2, 1.0, n, edge_separator, edge, order);
}
